...
import pandas as pd
import logging
from stock_choice.strategy import enter
...
def check(code_name, data, end_date=None, threshold=60):
    ...
    begin_date = data.iloc[0].date_day
    ...
    for index, row in data.iterrows():
        if row['open'] < row['ma60'] <= row['close']:
            if enter.check_volume(code_name, origin_data, row['date_day'], threshold):
                breakthrough_row = row

    if breakthrough_row is None:
        return False

    data_front = data.loc[(data['date_day'] < breakthrough_row['date_day'])]
    data_end = data.loc[(data['date_day'] >= breakthrough_row['date_day'])]

    for index, row in data_front.iterrows():
        if not (-0.05 < (row['ma60'] - row['close']) / row['ma60'] < 0.2):
            return False

    # logging.info("股票{0} 突破日期：{1}".format(code_name, breakthrough_row['date_day']))
