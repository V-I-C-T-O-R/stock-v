...
def check(code_name, data, end_date=None, threshold=60):
    ...
    data_front = data.loc[(data['date_day'] < highest_row['date_day'])]
    data_end = data.loc[(data['date_day'] >= highest_row['date_day'])]
    ...
    date_diff = datetime.date(datetime.strptime(recent_lowest_row['date_day'], '%Y-%m-%d')) - \
                datetime.date(datetime.strptime(highest_row['date_day'], '%Y-%m-%d'))
