    ...
    for index, row in data.iterrows():
        try:
            if float(row['p_change']) > 9.5:
                if turtle_trade.check_enter(code_name, origin_data, row['date_day'], threshold):
...
def check_internal(code_name, data, limitup_row):
    ...
    limitup_end = data.loc[(data['date_day'] > limitup_row['date_day'])]
    ...
    logging.debug("股票{0} 涨停日期：{1}".format(code_name, limitup_row['date_day']))
